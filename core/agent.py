...
DO NOT invent or hallucinate tool names that aren't in the list."""},
    ...
    for i, task in enumerate(tasks):
        ...
        print("tool_name_called:"+ tool_name)
...
IMPORTANT: If this task needs to use text content from a previous task result, extract the FULL TEXT CONTENT from the previous result and summarize ( required ) it in three to four lines and use it.
...
Return only a JSON object with the updated parameters. Do not include any explanations."""},
        ...
        for key, value in parameters.items():
            if key not in ['tool_name', 'parameters', 'requires']:
                valid_params[key] = value

# Then call the function with valid parameters
        tool_result = function_to_call(**valid_params)
